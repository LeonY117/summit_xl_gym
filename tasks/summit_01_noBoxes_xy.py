...
class Summit(VecTask):
    ...
    def __init__(self, cfg, sim_device, graphics_device_id, headless):
        ...
        self.cfg["env"]["numObservations"] = 14 + 24 * self.max_num_boxes  # 65
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.reset_buf[:] = compute_summit_reward(
            self.obs_buf, self.progress_buf, self.reset_buf, self.max_episode_length, self.potentials, self.prev_potentials, self.dist_to_target, self.reached_target, self.summit_pos_tensor)
    ...
    def pre_physics_step(self, actions):
        ...
        # target_velocities[:, 0] = torch.ones_like(self.actions[:, 0])
        # target_velocities[:, 2] = torch.zeros_like(self.actions[:, 1])
        # target_velocities[:, 1] = torch.ones_like(self.actions[:, 0])
        # target_velocities[:, 3] = torch.zeros_like(self.actions[:, 1])

        # back_left, back_right, front_left, front_right
        target_velocities = self.actions * 1
        ...
        self.summit_vel_tensor[:, 2] = torch.zeros_like(self.potentials)
...
@ torch.jit.script
def compute_summit_reward(obs_buf, progress_buf, reset_buf, max_episode_length, potentials, prev_potentials, dist_to_target, reached_target, summit_pos_tensor):
    ...
    # penalty for flipping summit
    summit_z = summit_pos_tensor[:, 2]
    flipped = torch.where(summit_z > 0.45, torch.ones_like(
        potentials), torch.zeros_like(potentials))
    flip_cost = -flipped * 100

    # optional: dof at limit
    total_reward = progress_reward + time_cost + \
        reached_goal_reward + dist_reward + flip_cost
    ...
    reset = torch.where(flipped == 1, torch.ones_like(reset_buf), reset)
...
def compute_summit_observations(potentials, prev_potentials, num_envs, max_num_boxes, dt,
    ...
    summit_pos = summit_pos_tensor[:, 0:3]
    summit_vel = summit_vel_tensor[:, 0:3]
